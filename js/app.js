...
function renderStories(){
  ...
  html+=`<div class="story-item" onclick="openStudioStory()">
    <div class="story-ring" style="background:linear-gradient(135deg,var(--coral),#ff9a9a,var(--coral-dark))">
      <div class="story-avatar story-avatar-logo"><img src="${LOGO_B64.badge}" alt="SpaceMush"></div>
    </div>
...
function renderFeed(){
  ...
  if(!c) return false;
  // Feed order is controlled from data.js so content editors do not need to
  // edit the website engine. Projects are referenced by their stable id, not
  // by array position, so adding/reordering projects will not silently point
  // an existing feed post at the wrong project.
  ...
  mixed.forEach((item,position)=>{
    try{
      if(item.type==='project'){
        const projectIndex=mushData.findIndex(function(project){ return String(project.id)===String(item.id); });
        const m=projectIndex>=0 ? mushData[projectIndex] : null;
        if(m) parts.push(buildPostCard(m,projectIndex));
      }else{
        const opts=feedInfoPostOptions[item.key];
        if(opts) parts.push(buildInfoPostCard(item.key,opts));
      }
    }catch(err){
      console.error('SpaceMush feed post '+position+' failed:',err);
      // Keep the rest of the feed usable if one post contains bad data.
      parts.push(`<article class="mush-card sr"><div class="post-header"><div class="post-avatar-ring"><div class="post-avatar">SM</div></div><div class="post-info"><div class="post-handle">SpaceMush Architects</div><div class="post-subloc">Chennai</div></div></div><div class="post-image-wrap"><div class="info-carousel-wrap" style="display:flex;align-items:center;justify-content:center;padding:40px;text-align:center"><div><div class="info-slide-heading">SpaceMush Architects</div><div class="info-slide-body"><p>Small Spaces Deserve Design.</p></div></div></div></div></article>`);
    }
  });
...
const feedInfoPostOptions={
  welcome:{avatarText:'SM',handle:'spacemush_architects_chennai',subtitle:'📍 Chennai · Est. 2020',caption:'<span class="handle">spacemush_architects_chennai</span> Welcome to SpaceMush — where small spaces deserve design. <span class="hashtag">#SpaceMush</span> <span class="hashtag">#SmallSpacesDeserveDesign</span>',actionOnClick:'bookConsultationWhatsApp()',time:'Welcome'},
  whoSSR:{avatarText:'SSR',handle:'Architect SSR',subtitle:'Who is SSR? · Founder of SpaceMush',caption:'<span class="handle">Architect SSR</span> The story behind the architect who started SpaceMush around one simple belief: small spaces deserve design. <span class="hashtag">#ArchitectSSR</span> <span class="hashtag">#SpaceMush</span>',actionOnClick:'bookConsultationWhatsApp()'},
  whoWeAre:{avatarText:'SM',handle:'spacemush_architects_chennai',subtitle:'Who are behind these designs? · The SpaceMush team',caption:'<span class="handle">spacemush_architects_chennai</span> Meet the people behind SpaceMush — from founder-led design to architecture, documentation and project delivery. <span class="hashtag">#SpaceMushTeam</span>',actionOnClick:'bookConsultationWhatsApp()'},
  howToFindUs:{avatarText:'SM',handle:'spacemush_architects_chennai',subtitle:'Alwarpet, Chennai · Visit our studio',caption:'<span class="handle">spacemush_architects_chennai</span> Find the SpaceMush studio in Alwarpet, Chennai. <span class="hashtag">#SpaceMushStudio</span>',actionOnClick:"window.open('https://maps.app.goo.gl/myANNorCUc2HB3qeA','_blank')"},
  howWeDesign:{avatarText:'SM',handle:'spacemush_architects_chennai',subtitle:'How we design · From constraint to space',caption:'<span class="handle">spacemush_architects_chennai</span> We study the plot, people, light, ventilation, movement and budget — then turn constraints into design decisions. <span class="hashtag">#HowWeDesign</span>',actionOnClick:'bookConsultationWhatsApp()'},
  laPerleDetail:{avatarText:'M18',handle:'Mush_18_LaPerle',subtitle:'La Perle · Design detail',caption:'<span class="handle">Mush_18_LaPerle</span> Double-height spaces, skylights, a cantilever staircase and an open island kitchen shape this home. <span class="hashtag">#MUSH18</span> <span class="hashtag">#LaPerle</span>',actionOnClick:'openProject(2)'},
  meetShyam:{avatarText:'SH',handle:'Shyam Sundar',subtitle:'Sr. Architect · SpaceMush',caption:'<span class="handle">Shyam Sundar</span> Meet the senior architect helping translate ideas into thoughtful, buildable spaces. <span class="hashtag">#SpaceMushTeam</span>',actionOnClick:'bookConsultationWhatsApp()'},
  annaDetail:{avatarText:'M05',handle:'Mush_05_AnnaNagar',subtitle:'Anna Nagar · Design detail',caption:'<span class="handle">Mush_05_AnnaNagar</span> An L-shaped staircase, skylights, CNC jaalis and pivoting louvers help a tight linear plot breathe and connect. <span class="hashtag">#MUSH05</span> <span class="hashtag">#SmallSpacesDeserveDesign</span>',actionOnClick:'openProject(0)'},
  team:{avatarText:'SM',handle:'spacemush_architects_chennai',subtitle:'The SpaceMush Team · People behind the spaces',caption:'<span class="handle">spacemush_architects_chennai</span> Meet the people behind the spaces. <span class="hashtag">#SpaceMushTeam</span>',actionOnClick:'bookConsultationWhatsApp()'},
  philosophy:{avatarText:'SM',handle:'spacemush_architects_chennai',subtitle:'Our philosophy · Small Spaces Deserve Design',caption:'<span class="handle">spacemush_architects_chennai</span> Good design should not be reserved for large homes. Every inch matters. <span class="hashtag">#SmallSpacesDeserveDesign</span>',actionOnClick:'bookConsultationWhatsApp()'},
  behindDesign:{avatarText:'SM',handle:'spacemush_architects_chennai',subtitle:'Behind the design · Site to finished space',caption:'<span class="handle">spacemush_architects_chennai</span> Architecture continues beyond the drawing: coordination, documentation, site decisions and construction all shape the finished space. <span class="hashtag">#BehindTheDesign</span>',actionOnClick:'bookConsultationWhatsApp()'},
  contact:{avatarText:'SM',handle:'spacemush_architects_chennai',subtitle:'💬 Start a conversation',caption:'<span class="handle">spacemush_architects_chennai</span> Have a space in mind? Tell us about it. Let’s talk. <span class="hashtag">#ContactSpaceMush</span>',actionOnClick:'bookConsultationWhatsApp()',time:'Start a conversation'}
};

// These informational posts use the same media treatment as project posts,
// including real SpaceMush people/project imagery wherever available.
function configureMixedInfoPosts(){
  const photo=(src,alt,extra='')=>`<img src="${src}" alt="${alt}" style="width:100%;height:100%;object-fit:cover;display:block;${extra}">`;
  const portrait=(src,alt,name,role)=>`<div style="height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:28px;box-sizing:border-box;text-align:center"><img src="${src}" alt="${alt}" style="width:180px;height:180px;border-radius:50%;object-fit:cover;border:3px solid rgba(255,255,255,.35);margin-bottom:18px"><div class="info-slide-heading">${name}</div><div class="info-slide-body"><p><strong>${role}</strong></p></div></div>`;
  const projectPhoto=(m,idx,alt)=>m && m.images && m.images.length ? photo(m.images[idx] || m.images[0],alt) : defaultSlideInner('🏠',alt,'<p>SpaceMush project detail.</p>');

  infoPosts.welcome={idx:0,slides:[
    {inner:`<div style="height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:32px;text-align:center;box-sizing:border-box"><div style="font-size:54px;margin-bottom:18px">🍄</div><div class="info-slide-heading">Welcome to SpaceMush</div><div class="info-slide-body"><p><strong>Chennai · Est. 2020</strong></p><p>Small spaces deserve design.</p></div></div>`}
  ]};

  infoPosts.whoSSR={idx:0,slides:[
    {inner:portrait(FOUNDER_B64.ssr,'Architect SSR','Architect SSR','Founder · SpaceMush Architects')},
    {inner:defaultSlideInner('','How SSR started SpaceMush','<p><strong>Ar. Sivaraman, aka SSR,</strong> is a domain expert in Design &amp; Construction with <strong>18+ years of experience</strong> and <strong>350+ projects</strong>, including work across esteemed firms in India and London.</p><p>SpaceMush is his brainchild — shaped by the idea that the same ambition and quality of design found in large projects should also be possible in small spaces.</p><p>The name itself came from his 7-year-old daughter Saraa, who dreams of designing a cute mushroom house. That idea became the starting point for a practice built around one belief: <strong>Every small space deserves design.</strong></p>')}
  ]};

  infoPosts.whoWeAre={idx:0,slides:[
    {inner:`<div style="height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:24px;box-sizing:border-box;text-align:center"><img src="${FOUNDER_B64.ssr}" alt="Architect SSR" style="width:180px;height:180px;border-radius:50%;object-fit:cover;border:3px solid rgba(255,255,255,.35);margin-bottom:16px"><div class="info-slide-heading">Architect SSR</div><div class="info-slide-body"><p><strong>Founder · SpaceMush Architects</strong></p><p>Founder of SpaceMush, bringing 18+ years of experience in design and construction and a practice built around the belief that every small space deserves design.</p></div></div>`},
    {inner:portrait(FOUNDER_B64.shyam,'Ar. Shyam Sundar','Ar. Shyam Sundar','Principal Architect')},
    {inner:defaultSlideInner('','Ar. Subash','<p><strong>Sr. Architect</strong></p><p>Supports architectural design development, documentation and project coordination.</p>')},
    {inner:defaultSlideInner('','Ar. Archana','<p><strong>Jr. Architect</strong></p><p>Supports design development, documentation and project coordination.</p>')},
    {inner:defaultSlideInner('','Ar. Aarthi','<p><strong>Jr. Architect</strong></p><p>Supports design development, documentation and project coordination.</p>')},
    {inner:defaultSlideInner('','Ar. Arun','<p><strong>Jr. Architect</strong></p><p>Supports design development, documentation and project coordination.</p>')}
  ]};

  infoPosts.howWeDesign={idx:0,slides:[
    {inner:projectPhoto(mushData[2],0,'La Perle — architecture')},
    {inner:projectPhoto(mushData[0],0,'Anna Nagar — architecture')},
    {inner:defaultSlideInner('📐','Constraint → Design Decision','<p>We start with the site, the people, the light, ventilation, movement and budget — then design around what matters.</p>')}
  ]};

  infoPosts.howToFindUs={idx:0,slides:[
    {inner:defaultSlideInner('📍','How to Find Us','<p><strong>55, A3 Swati Sahana Apartments</strong><br>2nd Floor, KB Dasan Road<br>Alwarpet, Chennai – 600018</p><p>Corner building on KB Dasan Road. From SIET College Road, it is on your LEFT. From TTK Road, it is on your RIGHT.</p><p>Ground-floor identifiers: <strong>Dakshin Art Gallery</strong> and <strong>Dental Clinic</strong>.</p><button class="info-slide-btn" onclick="window.open(\'https://maps.app.goo.gl/myANNorCUc2HB3qeA\',\'_blank\')">📍 Open in Google Maps</button>')},
    {inner:`<div style="height:100%;display:flex;align-items:center;justify-content:center;padding:18px;box-sizing:border-box"><img src="${OFFICE_B64.parking_guidance}" alt="SpaceMush office parking and location map" style="width:100%;height:100%;object-fit:contain;border-radius:12px"></div>`}
  ]};

  infoPosts.contact={idx:0,slides:[
    {inner:`<div class="info-slide-icon">💬</div><div class="info-slide-heading">Contact SpaceMush</div><div class="info-slide-body"><p>Have a space in mind? Tell us about your site, your requirements and what you want the space to become.</p></div><div class="info-slide-tiles"><div class="info-slide-tile" onclick="window.open('https://wa.me/919080430134','_blank')"><div class="info-slide-tile-icon">💬</div><div class="info-slide-tile-title">WhatsApp</div><div class="info-slide-tile-sub">+91 90804 30134</div></div><div class="info-slide-tile" onclick="window.location.href='mailto:ssr@spacemush.com'"><div class="info-slide-tile-icon">✉️</div><div class="info-slide-tile-title">Email</div><div class="info-slide-tile-sub">ssr@spacemush.com</div></div><div class="info-slide-tile" onclick="window.location.href='tel:+919080430134'"><div class="info-slide-tile-icon">📞</div><div class="info-slide-tile-title">Call Us</div><div class="info-slide-tile-sub">Mon–Sat 10am–7pm</div></div><div class="info-slide-tile" onclick="window.open('https://www.instagram.com/spacemush_architects_chennai/','_blank')"><div class="info-slide-tile-icon">📸</div><div class="info-slide-tile-title">Instagram</div><div class="info-slide-tile-sub">@spacemush_architects_chennai</div></div></div>`}
  ]};

  infoPosts.faq={idx:0,slides:[
    {inner:defaultSlideInner('❓','What does SpaceMush do?','<p>We design and execute small spaces — from new homes on constrained urban plots to renovations and interiors.</p>')},
    {inner:defaultSlideInner('🏗️','Do you only provide design?','<p>No. We offer <strong>Design Only</strong>, <strong>Design + Documentation</strong>, and <strong>Full End-to-End</strong> services depending on what you need.</p>')},
    {inner:defaultSlideInner('📐','What kind of projects do you take?','<p>We work on small and constrained residential spaces, including new homes, renovations and interior projects.</p>')},
    {inner:defaultSlideInner('📍','Where is SpaceMush based?','<p>Our studio is in <strong>Alwarpet, Chennai</strong>. See the “How to Find Us” post for the address and parking guidance.</p>')},
    {inner:defaultSlideInner('💬','How do I start a project?','<p>Send us your site details, requirements and what you have in mind. You can reach SpaceMush through WhatsApp, email or phone.</p><button class="info-slide-btn" onclick="bookConsultationWhatsApp()">WhatsApp SpaceMush</button>')}
  ]};
}
...
function buildPostCard(m,i){
  ...
  return `<article class="mush-card sr" data-index="${i}" data-cat="${m.category}">
    <div class="post-header">
      <div class="post-avatar-ring"><div class="post-avatar">${mushNumber(m)?'MUSH '+mushNumber(m):'SM'}</div></div>
      <div class="post-info">
    ...
    <button class="view-comments" onclick="openProject(${i})">View all ${m.comments} comments</button>
...
function buildProjectPostCard(m,i){
  ...
  return `<article class="mush-card sr" data-index="${i}" data-cat="${m.category}">
    <div class="post-header">
      <div class="post-avatar-ring"><div class="post-avatar">${m.id}</div></div>
      <div class="post-info">
        <div class="post-handle" onclick="openProject(${i})" style="cursor:pointer" title="View ${projectDisplayName(m)}">${projectDisplayName(m)}
          <div class="verified-badge"><svg viewBox="0 0 10 10"><polyline points="2,5 4,7 8,3" stroke="white" stroke-width="1.5" fill="none"/></svg></div>
        </div>
...
function buildInfoPostCard(key,opts){
  return `<article class="mush-card sr" data-key="${key}">
    <div class="post-header">
      <div class="post-avatar-ring"><div class="post-avatar">${opts.avatarText}</div></div>
      <div class="post-info">
...
function openProject(i){
  ...
  if(pcl){
    if(!postComments.length){
      pcl.innerHTML='<div style="padding:20px 0;text-align:center;color:var(--text4);font-size:13px">No comments yet. Be the first to comment!</div>';
    } else {
...
// PAGE NAVIGATION
// ============================================================
function scrollToFeedPost(key){
  const post=document.querySelector('#feedContainer .mush-card[data-key="'+key+'"]');
  if(!post) return;
  post.scrollIntoView({behavior:'smooth', block:'start'});
}

function scrollToFirstProject(){
  const post=document.querySelector('#feedContainer .mush-card[data-index]');
  if(!post) return;
  post.scrollIntoView({behavior:'smooth', block:'start'});
}
