    ...
    infoPosts[key]={
      ...
      slides:Array.isArray(cfg.slides) && cfg.slides.length
        ? cfg.slides.map(renderSlide).map(function(inner,idx){
            ...
            return {inner:inner,formVariant:!!raw.formVariant,theme:raw.theme||''};
          })
        : [
...
function renderInfoCarousel(key){
  ...
  const frames=slides.map((s,si)=>`<div class="info-slide${s.formVariant?' info-slide-form':''}${s.theme?' info-slide-theme-'+s.theme:''}" id="infoFrame_${key}_${si}" style="display:${si===idx?'flex':'none'}">${s.inner}</div>`).join('');
